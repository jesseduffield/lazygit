...
warning. Fixing one of the two commands alone would leave git's own diff
reporting every line as changed. And an external diff renderer is handed the
two files as they stand, one line ending apart in every line, with whatever it
makes of that being its own business.
...
Confirmed on Windows the same day: the warning is gone, and the setting in force
was `core.autocrlf=true` in the system config Git for Windows installs
(`C:/Program Files/Git/etc/gitconfig`), with `core.eol`, `core.safecrlf` and
`core.attributesfile` all unset. The trees being a line ending apart had cost
nothing visible even before the fix, under delta and under difftastic alike, so
...
- **2026-09-20 (later):** **PR 8 round 2**, on a warning above the custom patch
  ...
  converts their line endings by `core.autocrlf`, the setting Git for Windows
  ...
  above it. An external diff renderer is handed the two files themselves, one
  line ending apart, though delta and difftastic both rendered the preview
  normally all the same. One `fixup!` on "Materialize the custom patch so the
  diff renderer can show it" puts `NoLineEndingConversion()` on both commands,
  with `custom_patch_ignores_line_ending_conversion` reaching them through
  ...
  `jump-to-file-from-diffstat-2026-09-20-1912-backup`. Verified on Windows the
  same day, `core.autocrlf` being true in the system config there; the one case
  left is kept until someone reports it (§8 row).
