_Esse arquivo é gerado automaticamnete. Para atualizar, faça mudanças no pkg/i18n directorio e rode o comando `go generate ./...` do projeto raiz._

# Lazygit Keybindings

_Legenda: `<c-b>` significa ctrl+b, `<a-b>` significa alt+b, `B` significa shift+b_

## Global keybindings

| Key | Action | Info |
|-----|--------|-------------|
| `` <c-r> `` | Trocar para um recente repo |  |
| `` <pgup> (fn+up/shift+k) `` | Rolar para cima da página |  |
| `` <pgdown> (fn+down/shift+j) `` | Rolar para baixo da página |  |
| `` @ `` | Visuaizar o registro de opções de comandos | Visualizar opções para o registro de comandos ex. mostrar/esconder o registro de comandos e focar o registo de comandos. |
| `` P `` | Push | Emburrar para a branch atual (local) para a branch upstream (remota). Se não estiver uma upstream configurado, você será solicitado para configurar uma branch upstream. |
| `` p `` | Pull | Puxar mudanças do remoto para a branch atual. Se não estiver uma upstream configurado, você será solicitado para configurar uma branch upstream. |
| `` } `` | Aumentar o tamanho do contexto de comparação. | Aumentar o montante do contexto a cerca de mudanças na visualização da comparação. |
