...
func NewLocalCommitsContext(c *ContextCommon) *LocalCommitsContext {
	...
	getNonModelItems := func() []*NonModelItem {
		...
		if c.Model().WorkingTreeStateAtLastCommitRefresh.CanShowTodos() {
			if c.Model().WorkingTreeStateAtLastCommitRefresh.IsRebasing() {
			...
			if c.Model().WorkingTreeStateAtLastCommitRefresh.IsCherryPicking() ||
				c.Model().WorkingTreeStateAtLastCommitRefresh.IsReverting() {
				...
				label := lo.Ternary(c.Model().WorkingTreeStateAtLastCommitRefresh.IsCherryPicking(),
