...
func (gui *Gui) handleCommitFileSelect() error {
	...
	task := NewRunPtyTask(
		gui.Git.Diff().ShowFileDiffCmdObj(from, to, reverse, node.GetPath(), false, false),
	)
	...
	return gui.Ask(AskOpts{
		...
		HandleConfirm: func() error {
			return gui.WithWaitingStatus(gui.Tr.RebasingStatus, func() error {
				if err := gui.Git.WithSpan(gui.Tr.Spans.DiscardOldFileChange).Rebasing().DiscardOldFileChanges(gui.State.Commits, gui.State.Panels.Commits.SelectedLineIdx, fileName); err != nil {
...
func (gui *Gui) refreshCommitFilesView() error {
	...
	files, err := gui.Git.Diff().LoadDiffFiles(from, to, reverse)
	if err != nil {
		return gui.SurfaceError(err)
	}
