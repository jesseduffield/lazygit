...
func (gui *Gui) handleCreateTag() error {
	return gui.Prompt(PromptOpts{
		...
		HandleConfirm: func(tagName string) error {
			...
			if err := gui.Git.WithSpan(gui.Tr.Spans.CreateLightweightTag).Tags().LightweightCreate(tagName, ""); err != nil {
...
func (gui *Gui) handleDeleteTag(tag *models.Tag) error {
	...
	return gui.Ask(AskOpts{
		...
		HandleConfirm: func() error {
			if err := gui.Git.WithSpan(gui.Tr.Spans.DeleteTag).Tags().Delete(tag.Name); err != nil {
				return gui.SurfaceError(err)
			}
