...
func (gui *Gui) GetInitialKeybindings() ([]*types.Binding, []*gocui.ViewMouseBinding) {
	...
	bindings := []*types.Binding{
		...
		// Keybindings to toggle panel visibility (not persisted)
		{
			ViewName: "",
			Key:      '!', // Shift+1 for status panel
			Handler: func() error {
				gui.ShowStatusPanel = !gui.ShowStatusPanel
				gui.c.Refresh(types.RefreshOptions{})
				return nil
			},
			Description: "Toggle status panel (panel 1)",
		},
		{
			ViewName: "",
			Key:      '@', // Shift+2 for files panel
			Handler: func() error {
				gui.ShowFilesPanel = !gui.ShowFilesPanel
				gui.c.Refresh(types.RefreshOptions{})
				return nil
			},
			Description: "Toggle files panel (panel 2)",
		},
		{
			ViewName: "",
			Key:      '#', // Shift+3 for branches panel
			Handler: func() error {
				gui.ShowBranchesPanel = !gui.ShowBranchesPanel
				gui.c.Refresh(types.RefreshOptions{})
				return nil
			},
			Description: "Toggle branches/remotes/tags panel (panel 3)",
		},
		{
			ViewName: "",
			Key:      '$', // Shift+4 for commits panel
			Handler: func() error {
				gui.ShowCommitsPanel = !gui.ShowCommitsPanel
				gui.c.Refresh(types.RefreshOptions{})
				return nil
			},
			Description: "Toggle commits/reflog panel (panel 4)",
		},
		{
			ViewName: "",
			Key:      '%', // Shift+5 for stash panel
			Handler: func() error {
				gui.ShowStashPanel = !gui.ShowStashPanel
				gui.c.Refresh(types.RefreshOptions{})
				return nil
			},
			Description: "Toggle stash panel (panel 5)",
		},
	}
