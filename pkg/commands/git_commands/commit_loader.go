...
func (self *CommitLoader) MergeRebasingCommits(commits []*models.Commit) ([]*models.Commit, error) {
	...
	if workingTreeState.IsCherryPicking() || workingTreeState.IsReverting() {
	...
	if workingTreeState.IsRebasing() {
...
func (self *CommitLoader) getHydratedSequencerCommits(workingTreeState enums.WorkingTreeState) ([]*models.Commit, error) {
...
func (self *CommitLoader) getConflictedSequencerCommit(workingTreeState enums.WorkingTreeState) *models.Commit {
	var shaFile string
	var action todo.TodoCommand
	if workingTreeState.IsCherryPicking() {
		shaFile = "CHERRY_PICK_HEAD"
		action = todo.Pick
	} else if workingTreeState.IsReverting() {
		shaFile = "REVERT_HEAD"
		action = todo.Revert
	} else {
		return nil
