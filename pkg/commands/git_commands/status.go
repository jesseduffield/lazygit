...
func (self *StatusCommands) WorkingTreeState() enums.WorkingTreeState {
	result := enums.WORKING_TREE_STATE_NONE
	isInRebase, _ := self.IsInRebase()
	if isInRebase {
		result |= enums.WORKING_TREE_STATE_REBASING
	}
	merging, _ := self.IsInMergeState()
	if merging {
		result |= enums.WORKING_TREE_STATE_MERGING
	}
	cherryPicking, _ := self.IsInCherryPick()
	if cherryPicking {
		result |= enums.WORKING_TREE_STATE_CHERRY_PICKING
	}
	reverting, _ := self.IsInRevert()
	if reverting {
		result |= enums.WORKING_TREE_STATE_REVERTING
	}
	return result
}
