...
func (c *OSCommand) StartPipeline(cmdObjs ...*CmdObj) (*Pipeline, io.ReadCloser, error) {
	...
	return &Pipeline{cmds: cmds, cmdStr: cmdStr}, reader, nil
}
...
// platforms where that does nothing, closing the output reader is what brings
// the pipeline down; see StartPipeline.
...
//
// The parent's ends have to be closed once the commands are running, which is
// what startPipeline does; see there for why it matters.
