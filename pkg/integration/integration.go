...
func generateSnapshot(dir string) (string, error) {
	...
	for _, cmdStr := range cmdStrs {
		...
		output, _ := osCommand.RunWithOutput(oscommands.NewCmdObjFromStr(cmdStr))
...
func getLazygitCommand(testPath string, rootDir string, record bool, speed float64, extraCmdArgs string) (*exec.Cmd, error) {
	...
	var recordingEnvVar string
	if record {
		recordingEnvVar = fmt.Sprintf("RECORD_EVENTS_TO=%s", replayPath)
	} else {
		recordingEnvVar = fmt.Sprintf("REPLAY_EVENTS_FROM=%s", replayPath)
	}

	cmdObj := oscommands.NewCmdObjFromStr(cmdStr).AddEnvVars(fmt.Sprintf("SPEED=%f", speed), recordingEnvVar)
