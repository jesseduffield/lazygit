	...
	Run: func(t *TestDriver, keys config.KeybindingConfig) {
		t.Views().Files().Focus().
			Lines(
				...
				Contains("   M").Contains("submodule1 (submodule)"),
				Contains("   M").Contains("submodule2 (submodule)"),
			).
			...
			Lines(
				Contains("▼ dir"),
				Contains("  ??").Contains("file").IsSelected(),
				Contains("   M").Contains("submodule1 (submodule)").IsSelected(),
				Contains("   M").Contains("submodule2 (submodule)"),
			).
			Press(keys.Universal.Remove).
			Tap(func() {
			...
			Lines(
				...
				Contains("  ??").Contains("file"),
				Contains("   M").Contains("submodule1 (submodule)").IsSelected(),
				Contains("   M").Contains("submodule2 (submodule)").IsSelected(),
			).
			Press(keys.Universal.Remove).
			Tap(func() {
				t.ExpectToast(Contains("Disabled: Multiselection not supported for submodules"))
