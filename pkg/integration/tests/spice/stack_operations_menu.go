...
var StackOperationsMenu = NewIntegrationTest(NewIntegrationTestArgs{
	Description:  "Open the stack operations menu from spice stacks view",
	ExtraCmdArgs: []string{},
		...
		t.Views().SpiceStacks().
			...
			Tap(func() {
				t.ExpectPopup().Menu().
					...
					Select(Contains("Restack branch")).
					Cancel()
			})
	},
})
