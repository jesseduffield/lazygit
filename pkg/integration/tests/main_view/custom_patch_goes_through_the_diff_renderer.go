...
var CustomPatchGoesThroughTheDiffRenderer = NewIntegrationTest(NewIntegrationTestArgs{
	...
	Run: func(t *TestDriver, keys config.KeybindingConfig) {
		...
		// The patch is a diff like any other, so the renderer has had it too. git worked
		// out its context, and that is where the unchanged line either side comes from.
		t.Views().Secondary().
			Content(Contains("RENDERED BY ME")).
			ContainsLines(
				Contains(" one"),
