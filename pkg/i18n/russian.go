...
Благодарю за использование lazygit! Серьёзно, вы просто супер. Три вещи, которыми я хочу поделиться:
 ...
 3) Используете git? Значит Вы программист! С Вашей помощью мы можем сделать lazygit лучше,
    станьте участником и присоединитесь к веселью на
...
func RussianTranslationSet() TranslationSet {
	return TranslationSet{
		...
		SureForceCheckout:                   "Вы уверены, что хотите переключить принудительно? Вы потеряете все локальные изменения",
		...
		ForceDeleteBranchMessage:            "'{{.selectedBranchName}}' не полностью слилась. Вы уверены, что хотите удалить её?",
		...
		Pop:                                 "Применить спрятанные изменения и тут же удалить их из хранилища",
		Drop:                                "Удалить спрятанные изменения из хранилища",
		Apply:                               "Применить спрятанные изменения",
		...
		StashPop:                            "Применить спрятанные изменения и тут же удалить их из хранилища",
		SurePopStashEntry:                   "Вы уверены, что хотите применить эти спрятанные изменения и тут же удалить их из хранилища?",
		StashApply:                          "Применить спрятанные изменения",
		SureApplyStashEntry:                 "Вы уверены, что хотите применить эти спрятанные изменения?",
		...
		ForcePush:                           "Принудительная отправка изменений",
		ForcePushPrompt:                     "Ветка отклонилась от удалённой ветки. Нажмите «esc», чтобы отменить, или «enter», чтобы начать принудительную отправку изменений.",
		ForcePushDisabled:                   "Ветка отклонилась от удалённой ветки. Принудительная отправка изменений была отключена",
		UpdatesRejectedAndForcePushDisabled: "Обновления были отклонены. Принудительная отправка изменений была отключена",
		...
		NoBranchOnRemote:                    `Этой ветки не существует в удалённом репозитории. Сначала её нужно отправить в удалённый репозиторий.`,
		Fetch:                               `Получить изменения`,
		NoAutomaticGitFetchTitle:            `Нет автоматического получения изменений`,
		...
		NotMergingOrRebasing:                "В данный момент вы не выполняете ни перебазирование, ни слияние",
		...
		SimpleRebase:                        "Простое перебазирование",
		InteractiveRebase:                   "Интерактивное перебазировние",
		InteractiveRebaseTooltip:            "Начать интерактивное перебазирование с перерывом в начале, чтобы можно было обновить TODO коммиты, прежде чем продолжить.",
		ConfirmMerge:                        "Вы уверены, что хотите слить '{{.selectedBranch}}' с '{{.checkedOutBranch}}'?",
		FwdNoUpstream:                       "Невозможно перемотать ветку без upstream-ветки",
		FwdNoLocalUpstream:                  "Невозможно перемотать ветку. Удалённый репозиторий не зарегистрирован локально",
		...
		PullingStatus:                       "Получение и слияние изменений",
		PushingStatus:                       "Отправка изменений",
		FetchingStatus:                      "Получение изменений",
		...
		LowercaseRebasingStatus:             "перебазирование", // lowercase because it shows up in parentheses
		...
		CommittingStatus:                    "Сохранение изменений",
		...
		DiscardAllChangesToAllFiles:         "Уничтожить рабочее дерево?",
		...
		StashAllChanges:                     "Спрятать все изменения",
		StashStagedChanges:                  "Спрятать проиндексированные изменения",
		StashAllChangesKeepIndex:            "Спрятать все изменения и сохранить индекс",
		StashUnstagedChanges:                "Спрятать непроиндексированные изменения",
		StashIncludeUntrackedChanges:        "Спрятать все изменения, включая неотслеживаемые файлы",
		...
		NoPatchError:                        "Патч ещё не создан. Чтобы начать сборку патча, используйте «пробел» в файле коммита или «enter», чтобы добавить определённые строки.",
		...
		InvalidUpstream:                     "Недействительный upstream. Он должен быть в формате '<remote> <branchname>'",
		ReturnToRemotesList:                 `Вернуться к списку удалённых репозиториев`,
		...
		EditRemoteName:                      `Введите новое название для удалённой ветки {{.remoteName}}:`,
		EditRemoteUrl:                       `Введите новую ссылку для удалённой ветки {{.remoteName}}:`,
		...
		EditRemote:                          "Редактировать удалённый репозиторий",
		...
		PushTagTitle:                        "Удалённый репозиторий для отправки тега '{{.tagName}}' в:",
		PushTag:                             "Отправить тег",
		CreateTag:                           "Создать тег",
		FetchRemote:                         "Получение изменений из удалённого репозитория",
		...
		RenameBranchWarning:                 "Эта ветвь отслеживает удалённый репозиторий. Это действие переименует только имя локальной ветки, а не имя удалённой ветки. Продолжить?",
		...
		MustStashTitle:                      "Необходимо спрятать",
		...
		SwapDiff:                            "Обратное направление сравнений",
		OpenDiffingMenu:                     "Открыть меню сравнений",
		...
		ToggleWhitespaceInDiffView:          "Переключить отображение изменений пробелов в просмотрщике сравнений",
		...
		IncreaseContextInDiffView:           "Увеличить размер контекста, отображаемого вокруг изменений в просмотрщике сравнений",
		DecreaseContextInDiffView:           "Уменьшите размер контекста, отображаемого вокруг изменений в просмотрщике сравнений",
		...
		MustSpecifyOriginError:              "Необходимо указать удалённый репозиторий, если указываете ветку",
		GitOutput:                           "Вывод git:",
		...
		Actions: Actions{
			...
			OpenCommitInBrowser:               "Открыть коммит в браузере",
			OpenPullRequest:                   "Открыть запрос на принятие изменений в браузере",
		...
		Bisect: Bisect{
			...
			CompletePromptIndeterminate: "Бинарный поиск завершён! Некоторые коммиты были пропущены, поэтому любой из следующих коммитов мог внести изменения::\n\n%s\n\nСбросить 'git bisect' сейчас?",
