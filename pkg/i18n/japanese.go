...
func japaneseTranslationSet() TranslationSet {
	return TranslationSet{
		...
		CredentialsPIN:                      "SSH鍵のPINを入力してください",
		...
		SureForceCheckout:                   "ローカルの変更を全て失いますが、本当に強制的にcheckoutをしますか？",
		ForceCheckoutBranch:                 "ブランチを force checkout",
		BranchName:                          "ブランチ名",
		...
		LcResetCommitAuthor:                 "コミットのauthorをリセット",
		SetAuthorPromptTitle:                "authorをセット ('Name <Email>'形式)",
    // SureResetCommitAuthor:               "このコミットのauthor欄は設定されたユーザーと一致するようにセットされ、author timestampも更新されます. 続けますか?",
		...
		UndoTooltip:                         "reflogは直近のgitコマンドをアンドゥするコマンドを決定するために使用されます。これはワーキングツリーへの変更を含まず、コミットのみが考慮されます。",
		RedoTooltip:                         "reflogは直近のgitコマンドをリドゥするコマンドを決定するために使用されます。これはワーキングツリーへの変更を含まず、コミットのみが考慮されます。",
		DiscardAllTooltip:                   "'{{.path}}'内のステージ済/未ステージ双方の変更を破棄する。",
		DiscardUnstagedTooltip:              "'{{.path}}'内のステージされていない変更を破棄する。",
		LcPop:                               "pop",
		LcDrop:                              "drop",
		LcApply:                             "適用",
		...
		NoTrackedStagedFilesStash:           "stashのための追跡/ステージ済ファイルがありません",
		NoFilesToStash:                      "stashするファイルがありません",
		...
		LcExcludeFile:                       `.git/info/excludeに追加`,
		...
		FileEnter:                           `ファイルの個々のhunk/行、またはそれぞれのディレクトリをステージする`,
		FileStagingRequirements:             `個々の行か追跡済ファイルのみをステージできます`,
		...
		FastForward:                         `このブランチをupstreamへfast-forwardする`,
		Fetching:                            "{{.from}} から {{.to}} へfetch/fast-foward中...",
		FoundConflicts:                      "コンフリクトが発生しました! 'esc'で中断、'enter'で続行",
		FoundConflictsTitle:                 "自動マージに失敗",
		PickHunk:                            "hunkを選択",
		...
		FwdCommitsToPush:                    "pushされていないコミットがあるブランチはfast-fowardできません",
		ErrorOccurred:                       "エラーが発生しました! issueを作成してください: ",
		// NoRoom:                              "十分な空きがありません",
		YouAreHere:                          "現在位置",
		YouDied:                             "YOU DIED!",
		LcRewordNotSupported:                "インタラクティブrebase中のコミットの編集は現在サポートされていません",
		...
		LcCheckoutCommitFile:                "ファイルをcheckout",
		LcDiscardOldFileChange:              "ファイルに対するこのコミット中の変更を破棄",
		DiscardFileChangesTitle:             "ファイルの変更を破棄",
		DiscardFileChangesPrompt:            "このコミット中のファイルに対する変更を破棄してもよろしいですか? このコミットでファイルが作成されていた場合、ファイルは削除されます",
		DisabledForGPG:                      "GPGを使用している場合、この機能は利用できません",
		CreateRepo:                          "Gitリポジトリではありません。リポジトリを作成しますか? (y/n): ",
		BareRepo:                            "Lazygitはまだbareリポジトリをサポートしていません。直近で開いたリポジトリを開きますか? (y/n) ",
		InitialBranch:                       "ブランチ名を入力 (空の場合、gitのデフォルトを使用します): ",
		NoRecentRepositories:                "LazygitはGitリポジトリの中で開く必要があります。直近で開いたリポジトリがないため終了します。",
		IncorrectNotARepository:             "'notARepository'の値が誤っています。'prompt', 'create', 'skip', 'quit'のいずれかである必要があります。",
		...
		LcCancel:                            "キャンセル",
		LcDiscardAllChanges:                 "すべての変更を完全に破棄",
		LcDiscardUnstagedChanges:            "未ステージの変更を破棄",
		LcDiscardAllChangesToAllFiles:       "ワーキングツリーを破棄",
		LcDiscardAnyUnstagedChanges:         "ステージされていない変更を破棄する",
		LcDiscardUntrackedFiles:             "未追跡ファイルの変更を破棄",
		LcDiscardStagedChanges:              "ステージされた変更を破棄",
		LcHardReset:                         "hardリセット",
		LcViewResetOptions:                  `「リセット」オプションを開く`,
		LcCreateFixupCommit:                 `このコミットに対するfixupコミットを作成`,
		LcSquashAboveCommits:                `選択したコミットより上の全ての'fixup!'コミットをsquashする(autosquash)`,
		SquashAboveCommits:                  `選択したコミットより上の全ての'fixup!'コミットをsquashする(autosquash)`,
		...
		LcStashAllChangesKeepIndex:          "全ての変更をstashしてindexを保持する",
		LcStashUnstagedChanges:              "未ステージの変更をstash",
		LcStashIncludeUntrackedChanges:      "未追跡のファイルを含む全ての変更をstashする",
		...
		LcSetAsUpstream:                     "checkout中ブランチのupstreamにセット",
		LcSetUpstream:                       "checkout中ブランチのupstreamにセット",
		LcUnsetUpstream:                     "選択したブランチのupstreamを解除",
		...
		IgnoreTracked:                       "追跡中のファイルをignore",
		IgnoreTrackedPrompt:                 "追跡済ファイルをignoreします。よろしいですか?",
		ExcludeTracked:                      "追跡済ファイルを除外",
		ExcludeTrackedPrompt:                "追跡済ファイルを除外してもよろしいですか?",
		...
		LcSetUnsetUpstream:                  "upstreamをセット/解除",
		...
		LcFilterPathOption:                  "filterするパスを入力",
		EnterFileName:                       "パスを入力:",
		...
		PatchCopiedToClipboard:              "パッチはクリップボードにコピーされました",
		...
		NukeDescription:                     "ワーキングツリーに行った全ての変更は破棄されます。dirtyサブモジュールへの変更はstashされます。",
		DiscardStagedChangesDescription:     "ステージ済ファイルのみを含む新しいstashエントリが作成されdropされます、これによりワーキングツリーには未ステージのファイルのみ残されます。",
		EmptyOutput:                         "<empty output>",
		Patch:                               "パッチ",
		CustomPatch:                         "カスタムパッチ",
		LcCommitsCopied:                     "コミットがコピーされました",
		LcCommitCopied:                      "コミットがコピーされました",
		Actions: Actions{
			// TODO: combine this with the original keybinding descriptions (those are all in lowercase atm)
			CheckoutCommit:                    "コミットをcheckout",
			CheckoutTag:                       "タグをcheckout",
			CheckoutBranch:                    "ブランチをcheckout",
			ForceCheckoutBranch:               "ブランチをforce checkout",
			DeleteBranch:                      "ブランチを削除",
			Merge:                             "マージ",
			RebaseBranch:                      "rebase branch",
			RenameBranch:                      "ブランチ名を変更",
			SetUnsetUpstream:                  "upstreamをセット/解除",
			CreateBranch:                      "ブランチを作成",
			CherryPick:                        "(cherry-pick)コミットを貼り付け",
			CheckoutFile:                      "ファイルをcheckout",
			DiscardOldFileChange:              "古いファイルの変更を破棄",
			SquashCommitDown:                  "下のコミットにsquashする",
			FixupCommit:                       "fixupコミット",
			RewordCommit:                      "コミットメッセージを編集",
			DropCommit:                        "コミットを削除",
			EditCommit:                        "コミットを編集",
			AmendCommit:                       "amendコミット",
			ResetCommitAuthor:                 "コミットauthorをリセット",
			SetCommitAuthor:                   "コミットauthorをセット",
			RevertCommit:                      "コミットをrevert",
			CreateFixupCommit:                 "fixupコミットを作成",
			SquashAllAboveFixupCommits:        "以上全てのfixupコミットをsquashする",
			...
			CopyCommitAuthorToClipboard:       "コミットauthorをクリップボードにコピー",
			...
			IgnoreFileErr:                     ".gitignore を ignore することはできません",
			ExcludeFile:                       "ファイルを除外",
			ExcludeFileErr:                    ".git/info/exclude を除外することはできません",
			ExcludeGitIgnoreErr:               ".gitignore を除外することはできません",
			Commit:                            "コミット",
			EditFile:                          "ファイルを編集",
			Push:                              "push",
			Pull:                              "pull",
			...
			StashAllChangesKeepIndex:          "全ての変更をstashしてindexを保持する",
			StashStagedChanges:                "ステージされた変更をstash",
			StashUnstagedChanges:              "未ステージの変更をstash",
			StashIncludeUntrackedChanges:      "未追跡のファイルを含む全ての変更をstashする",
			...
			DeleteRemoteBranch:                "リモートブランチを削除",
			SetBranchUpstream:                 "upstreamブランチをセット",
			AddRemote:                         "リモートを追加",
			...
			RemoveUntrackedFiles:              "未追跡のファイルを削除",
			RemoveStagedFiles:                 "ステージ済ファイルを削除",
			SoftReset:                         "softリセット",
		...
		Bisect: Bisect{
			...
			CompletePrompt:              "bisectが完了しました! 以下のコミットで変更が行なわれた可能性があります:\n\n%s\n\n今すぐ'git bisect'をリセットしますか?",
